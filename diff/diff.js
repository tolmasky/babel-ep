...
const toGeneratedCodeAtPath = (path, AST) =>
    generate(path.reduce((object, key) => object[key], AST)).code;
...
class Difference {
  constructor(adjust, expected, actual) {
    ...
    console.log(path);
    const message =
        `${toExplanationString(reason)} in ${toPathString(path)}:\n` + 
        `${toGeneratedCodeAtPath(path.slice(0, -1), expected)}\n` +
        `${toGeneratedCodeAtPath(path.slice(0, -1), actual)}`;
...
const toType = value =>
  value === null
    ...
    ? "Error"
    : "Object";
const STANDARD  = x => x.filter(x => x !== "end" && x !== "start" && x !== "extra");
...
function compare(adjust, expected, actual) {
  ...
  const keysExpected = STANDARD(Object.keys(expected));
  const keysActual = STANDARD(Object.keys(actual)).filter(
...
module.exports = (lhs, rhs) => new Difference(false, lhs, rhs);
